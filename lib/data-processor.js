...
import { deriveStatus, toDateKey, extractWarehouse, extractPrimaryWarehouse, diffCategory } from './helpers.js';
...
function enrichRow(raw, diffIdx) {
  ...
  const primaryWarehouse = extractPrimaryWarehouse(raw[C.GATE_T2], raw[C.GATE_T3]);
...
export function computeKPIs(allData, shortageData, diffRows) {
  ...
  const clearTodayList = shortageData.filter(d =>
    d.diffSaveTime && d.diffSaveTime.toISOString().slice(0, 10) === today
  );
  const notClearedList = shortageData.filter(d => d.r008 !== 'ไม่ขาดได้ครบ');
  const notCleared     = notClearedList.length;
  const clearedCasesList   = allData.filter(d => d.r008Reason && d.scanShort === 0 && d.scanOver === 0);
  const clearedCases       = clearedCasesList.length;
  const clearedSameDayList = allData.filter(d =>
    d.r008Reason && d.scanShort === 0 && d.scanOver === 0 &&
    d.r008Date && d.arriveDate &&
    toDateKey(d.r008Date) === toDateKey(d.arriveDate)
  );
  const clearedSameDay = clearedSameDayList.length;

  const clearedCrossDayList = allData.filter(d =>
    d.r008Reason && d.scanShort === 0 && d.scanOver === 0 &&
    d.r008Date && d.arriveDate &&
    toDateKey(d.r008Date) !== toDateKey(d.arriveDate)
  );
  const clearedCrossDay = clearedCrossDayList.length;

  const wh1Docs = shortageData.filter(d => d.primaryWarehouse === 'WH1').length;
  const wh2Docs = shortageData.filter(d => d.primaryWarehouse === 'WH2').length;
  const wh3Docs = shortageData.filter(d => d.primaryWarehouse === 'WH3').length;
  ...
  return {
    ...
    clearedCrossDay,
    clearedCrossDayList,
    // Per-warehouse shortage doc counts
    wh1Docs,
    wh2Docs,
    wh3Docs,
  };
