...
export function computeKPIs(allData, shortageData, diffRows) {
  ...
  const wh1Docs = shortageData.filter(d => d.warehouse.includes('WH1')).length;
  const wh2Docs = shortageData.filter(d => d.warehouse.includes('WH2')).length;
  const wh3Docs = shortageData.filter(d => d.warehouse.includes('WH3')).length;
