...
export function formatDate(val, includeTime = false) {
  ...
  else if (typeof val === 'string') d = new Date(val.replace(' ', 'T'));
...
/* Primary warehouse for counting — T3 first, fall back to T2 */
export function extractPrimaryWarehouse(t2, t3) {
  const prefix = gate => {
    if (!gate) return null;
    const s = String(gate).trim();
    const idx = s.lastIndexOf('-');
